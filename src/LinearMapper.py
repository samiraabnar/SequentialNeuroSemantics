...
class LinearMapper(object):
  ...
  def model(self, input, p_keep_input,
    ...
    h = tf.matmul(input, tf.nn.dropout(self.w_h,p_keep_hidden)) + self.b_h
    #h = tf.nn.dropout(h, p_keep_hidden)

    return h,h#tf.matmul(h, self.w_o) + self.b_o, h

  def build_mapping_model(self):
    if self.hparams.one_step == True:
      linear_steps = 1
    else:
    ...
    if self.hparams.weights == "avg":
      # self.input_states_batch_steps = tf.unstack(self.input_states_batch_steps)
      weighted_input_states_batch = [input_step for input_step, w_step in
                                     zip(tf.unstack(self.input_states_batch), tf.unstack(self.step_weights))]
    elif self.hparams.weights == "trained":
      # self.input_states_batch_steps = tf.unstack(self.input_states_batch_steps)
      weighted_input_states_batch = [w_step * input_step for input_step, w_step in
                                     zip(tf.unstack(self.input_states_batch), tf.unstack(self.step_weights))]
    ...
    with tf.variable_scope("hidden_layers"):
      self.w_h = self.init_weights(name="w_h", shape=[self.hparams.input_dim, self.hparams.output_dim])
      self.b_h = self.init_weights(name="b_h", shape=[self.hparams.output_dim], bias=True)
    ...
    self.mean_error, self.sd_error = tf.nn.moments(tf.subtract(self.predicted_output, self.output_states_batch),
                                                   axes=[1, 0])

    tf.summary.histogram("predicted_outputs", self.predicted_output)

    self.mean_squared_loss = tf.reduce_mean(
      tf.losses.mean_squared_error(labels=self.output_states_batch, predictions=self.predicted_output))

    self.pred_dists = tf.losses.mean_pairwise_squared_error(labels=self.predicted_output,
                                                            predictions=self.predicted_output)
    self.target_dists = tf.losses.mean_pairwise_squared_error(labels=self.output_states_batch,
                                                              predictions=self.output_states_batch)
    ...
    self.train_op = tf.train.AdamOptimizer(self.learning_rate).minimize(self.mean_squared_loss + self.l2_loss,
                                                                        global_step=self.global_step)
