...
class BrainAutoEncoder(object):
  ...
  def build_graph(self):
    ...
    self.mean_error, self.sd_error = tf.nn.moments(tf.subtract(self.predicted_output, self.input_states_batch),
                                                   axes=[1, 0])

    self.mean_squared_loss = tf.reduce_mean(
      tf.losses.mean_squared_error(labels=self.input_states_batch, predictions=self.decoded_output))
    ...
    self.train_op = tf.train.AdamOptimizer(self.learning_rate).minimize(self.mean_squared_loss,
                                                                        global_step=self.global_step)
...
def main(unused_argv):
  ...
  with tf.Graph().as_default():
    ...
    with sv.managed_session() as sess:
      if FLAGS.mode == "train":
        train(mapper, sess, sv, train_normalized_brain_scans, train_normalized_brain_scans, train_words, train_words,
              FLAGS=FLAGS, best_saver=best_saver, best_dir=best_dir)
